 ...
 * <ul>
 * <li>either for the active signal or for the signal from the given
 * document,</li>
 * <li>either from a {@link ChannelSamples single channel} or from
 * {@link SignalSamples all channels} in the signal,</li>
 * <li>either processed (after the montage and filtering) or raw
 * (unprocessed).</li>
 * </ul></li>
 ...
 * {@link ExportedTagDocument tag} documents or just active
 * {@link Document}</li>
 * <li>open:
 * <ul>
...
public interface SvarogAccessSignal {
	...
	/**
	 * Returns the processed signal samples (output of the
	 * SignalProcessingChain) for the active signal and the given channel.
	 *
	 * @param channel the index of the channel (range from 0 to
	 * {@code getActiveSignalDocument().getChannelCount()-1})
	...
	/**
	 * Returns the processed signal samples (output of the
	 * SignalProcessingChain) for all channels of the active signal.
	 *
	 * @return the signal samples
	 * @throws NoActiveObjectException if there is no active signal
	 */
	...
	/**
	 * Returns the processed signal samples (output of the
	 * SignalProcessingChain) for all channels of all signals.
	 *
	 * @return the signal samples
	 */
	...
	/**
	 * Returns the unprocessed signal samples (raw from the source) for the
	 * active signal and the given channel.
	 *
	 * @param channel the index of the channel (range from 0 to
	 * {@code getActiveSignalDocument().getChannelCount()-1})
	 * @return the signal samples
	 ...
	 * @param channel the index of the channel (range from 0 to
	 * {@code getActiveSignalDocument().getChannelCount()-1})
	 * @param signalOffset the number of the sample in the signal starting from
	...
	/**
	 * Returns the unprocessed signal samples (raw from the source) for all
	 * channels of the active signal.
	 *
	 * @return the signal samples
	 * @throws NoActiveObjectException if there is no active signal
	 */
	...
	/**
	 * Returns the unprocessed signal samples (raw from the source) for the
	 * signal from the given document and the given channel.
	 *
	 ...
	 * @param channel the index of the channel (range from 0 to
	 * {@code document.getChannelCount()-1})
	 * @return the signal samples
	 * @throws InvalidClassException if document is not returned from this
	 * SvarogAccess (not of type SignalDocument - internal to Svarog)
	 * @throws IndexOutOfBoundsException if the index of a channel is out of
	 * range
	 */
	...
	/**
	 * Returns the unprocessed signal samples (raw from the source) for all
	 * channels of the signal from the given document.
	 *
	 * @param document the document with the signal. Must be returned from this
	 * SvarogAcces (actually be of type SignalDocument - internal to Svarog).
	 * @return the signal samples
	...
	/**
	 * Returns the processed signal samples for the signal from the given
	 * document and the given channel.
	 *
	 * @param document the document with the signal. Must be returned from this
	 * SvarogAcces (actually be of type SignalDocument - internal to Svarog).
	 * @param channel the index of the channel (range from 0 to
	...
	/**
	 * Returns the processed signal samples for all channels of the signal from
	 * the given document.
	 *
	 * @param document the document with the signal. Must be returned from this
	 * SvarogAcces (actually be of type SignalDocument - internal to Svarog).
	...
	/**
	 * Returns the processed signal samples for the active signal and the given
	 * channel.
	 *
	 * @param channel the index of the channel (range from 0 to
	 * {@code getActiveSignalDocument().getChannelCount()-1})
	 ...
	 * @param channel the index of the channel (range from 0 to
	 * {@code getActiveSignalDocument().getChannelCount()-1})
	 * @param signalOffsetTime the position in the signal (in time domain)
	...
	/**
	 * Returns all {@link Tag tags} associated with the active tag document.
	 *
	...
	/**
	 * Returns all opened {@link Tag tags} (no matter with which document are
	 * associated).
	 *
	...
	/**
	 * Returns {@link Tag tags} associated with the given
	 * {@link ExportedSignalDocument}.
	 *
	...
	/**
	 * Returns the active {@link SignalSelection selection} of the part of the
	 * active signal.
	 *
	...
	/**
	 * Adds the {@link Tag tag} created form given {@link ExportedTag} to the
	 * active {@link TagDocument}. The tag has to have a
	 * {@link ExportedTagStyle} that exists in the active tag document (the set
	 * of styles can be obtained with
	...
	/**
	 * Adds the {@link Tag tag} created form given {@link ExportedTag} to the
	 * given {@link TagDocument}. The tag has to have a {@link ExportedTagStyle}
	 * that exists in the tag document (the set of styles can be obtained with
	 * {@code document.getTagStyles()}).
	 *
	 * @param document the document the tag will be added to. Must be returned
	 * from this SvarogAcces (actually be of type TagDocument - internal to
	 * Svarog).
	 * @param tag the tag to be added
	 * @throws InvalidClassException if document is not returned from this
	 * SvarogAccess (not of type TagDocument - internal to Svarog)
	 * @throws IllegalArgumentException if there is no such tag style (as the
	 * style of the tag) in the tag document
	 */
	...
	/**
	 * Returns the array of {@link TagDocument tag documents} associated with
	 ...
	 * {@link ExportedSignalDocument signal} or a book; or something else if it
	 * will be added).
	 *
	...
	/**
	 * Opens the signal from the given file as the raw signal. Probably some
	 ...
	 * @param calibration constant by which numbers from file are multiplied to
	 * get a physical value
	...
	/**
	 * Opens the signal from the given file using the given
	...
	/**
	 * Opens the tag document from the given file and adds it to the given
	 * signal document.
	 *
	 ...
	 *
	 * @param position the position in signal (seconds) where the exported part
	 * starts
	 ...
	 * @param level the {@link SignalSourceLevel level} of processing of the
	 * signal
	 * @param sampleType the {@link ExportedRawSignalSampleType type} of samples
	 * (short, int, float, double)
	 * @param byteOrder the {@link ByteOrder order} of bytes in which the signal
	 * should be written
	 * @param plot the {@link ExportedSignalPlot plot} for the signal. Must be
	 * returned from this SvarogAcces (actually be of type SignalPlot - internal
	 * to Svarog).
	...
	/**
	 * Creates and returns a temporary file. The file is created in the default
	 * temporary folder. When the File object is destroyed (by the Garbage
	 * Collector) the file is removed from disk. NOTE: it would be a good
	 * practice if you removed your temporary files when you finish using them.
	 * Especially if you use a lot of them or big ones.
	 *
