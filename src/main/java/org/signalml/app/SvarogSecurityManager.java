...
public class SvarogSecurityManager extends java.lang.SecurityManager {
    ...
    private HashMap<Thread,org.signalml.util.FastMutableInt> recLevel = new HashMap<Thread,org.signalml.util.FastMutableInt>();
    ...
    @Override
    /**
     * For top-level calls (no recursion) the permission (p) is granted iff at least 1 of the following holds:
     * 
     * <ul>
...
//    }
    ...
    protected String toString(StackTraceElement f) {
        return (f.getClassName() + "#" + f.getMethodName() + "(" + f.getFileName() + ":" + f.getLineNumber() + ")");
    }
}
