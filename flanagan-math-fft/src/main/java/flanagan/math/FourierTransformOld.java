...
public class FourierTransformOld { // extends Canvas {

	protected static final Logger logger = Logger.getLogger(FourierTransform.class);
...
//    private String shortTitle = " ";            // Short Time Fourier Transform graph title
	...
	public FourierTransformOld(Complex[] data){
    ...
    public Complex[] getComplexInputData(){
        if(!this.complexDataSet){
		    logger.debug("complex data set not entered or calculated - null returned");
		}
    ...
    public double[] getAlternateInputData(){
        if(!this.fftDataSet){
		    logger.debug("fft data set not entered or calculted - null returned");
		}
	    return this.fftData;
	}
    ...
    public double[] getAlternateWindowedInputData(){
		...
		if(!this.windowApplied){
		    logger.debug("fft data set has not been multiplied by windowing weights");
		}
    ...
    public double getDeltaT(){
        ...
        if(this.deltaTset){
            ...
            logger.debug("detaT has not been set - zero returned");
        }
        return ret;
    }
    ...
    public void setGaussian(double alpha){
        if(alpha<2.0D){
            alpha=2.0D;
            logger.debug("setGaussian; alpha must be greater than or equal to 2 - alpha has been reset to 2");
        }
	...
	private void checkSegmentDetails(){
        ...
        if(this.fftDataLength % 2 != 0){
	        logger.debug("Number of data points must be an even number");
	        logger.debug("last point deleted");
        ...
        if(this.segNumSet && !this.overlap){
            if(this.fftDataLength % this.segmentNumber == 0){
                ...
                if(FourierTransform.checkPowerOfTwo(segL)){
                    ...
                    logger.debug("segment length is not an integer power of two");
                    logger.debug("segment length reset to total data length, i.e. no segmentation");
                    warning[1] = true;
                ...
                logger.debug("total data length divided by the number of segments is not an integer");
                logger.debug("segment length reset to total data length, i.e. no segmentation");
                warning[2] = true;
                this.segmentNumber = 1;
                this.segmentLength = this.fftDataLength;
        ...
        if(this.segLenSet && !this.overlap){
            if(this.fftDataLength % this.segmentLength == 0){
                ...
                logger.debug("total data length divided by the segment length is not an integer");
                logger.debug("segment length reset to total data length, i.e. no segmentation");
                warning[3] = true;
                this.segmentNumber = 1;
                this.segmentLength = this.fftDataLength;
        ...
        if(this.segNumSet && this.overlap){
            if(this.fftDataLength % (this.segmentNumber+1) == 0){
                ...
                logger.debug("total data length divided by the number of segments plus one is not an integer");
                logger.debug("segment length reset to total data length, i.e. no segmentation");
                warning[4] = true;
                this.segmentNumber = 1;
                this.segmentLength = this.fftDataLength;
        ...
        if(this.segLenSet && this.overlap){
            if((2*this.fftDataLength) % this.segmentLength == 0){
                ...
                logger.debug("twice the total data length divided by the segment length is not an integer");
                logger.debug("segment length reset to total data length, i.e. no segmentation");
                warning[5] = true;
                this.segmentNumber = 1;
                this.segmentLength = this.fftDataLength;
        ...
        if(this.overlap && this.segmentNumber<2){
		    logger.debug("Overlap is not possible with less than two segments.");
			logger.debug("Overlap option has been reset to 'no overlap' i.e. to false.");
            this.overlap = false;
            this.segmentNumber = 1;
            this.segNumSet = true;
            warning[6] = true;
	    ...
	    if(this.segmentNumber==1){
	        ...
	        if(!FourierTransform.checkPowerOfTwo(this.fftDataLength)){
                ...
                if(setSegment && (this.originalDataLength-segL <= this.fftDataLength - this.originalDataLength)){
	                logger.debug("Data length is not an integer power of two");
	                logger.debug("Data cannot be transformed as a single segment");
	                logger.debug("The data has been split into " + segN+ " segments of length " + segL);
	                if(ovrlp){
	                    logger.debug(" with 50% overlap");
	                }
	                else{
	                    logger.debug(" with no overlap");
	                }
	                this.segmentLength = segL;
	                ...
	                logger.debug("Data length is not an integer power of two");
	                if(this.dataAltered){
	                    logger.debug("Deleted point has been restored and the data has been padded with zeros to give a power of two length");
	                    this.warning[0] = false;
	                }
	                else{
    	                logger.debug("Data has been padded with zeros to give a power of two length");
    	            }
	...
	*/
    ...
    public void transform(){
	    ...
	    if(this.originalDataLength!=this.fftDataLength){
	        logger.debug("Fast Fourier Transform data length ," + this.originalDataLength + ", is not an integer power of two");
	        logger.debug("WARNING!!! Data has been padded with zeros to fill to nearest integer power of two length " + this.fftDataLength);
	    }
...
*/
    ...
    public double[][] getpowerSpectrumEstimate(){
        if(!this.powSpecDone)logger.debug("getpowerSpectrumEstimate - powerSpectrum has not been called - null returned");
		return this.powerSpectrumEstimate;
	}
	...
	*/
    ...
    public void plotPowerSpectrum(int lowPoint, int highPoint, String graphTitle){
        if(!this.powSpecDone){
            logger.debug("plotPowerSpectrum - powerSpectrum has not been called - no plot displayed");
		}
		else{
    ...
    public void plotPowerSpectrum(double lowFreq, double highFreq, String graphTitle){
        if(!this.powSpecDone){
            ...
            int high = 0;
            if(!this.deltaTset){
                logger.debug("plotPowerSpectrum - deltaT has not been set");
                logger.debug("full spectrum plotted");
		    }
		    else{
		        int ii = 0;
    		    int n = this.powerSpectrumEstimate[0].length - 1;
    		    ...
    		    if(lowFreq==-1.0D){
		            ...
		            while(test){
    		            if(this.powerSpectrumEstimate[0][ii]>lowFreq){
    		                ...
    		                if(ii>=n){
	    	                    low = 0;
		                        logger.debug("plotPowerSpectrum - lowFreq out of range -  reset to zero");
    		                    test = false;
	    	                }
		                }
    		        }
		        ...
		        while(test){
		            if(this.powerSpectrumEstimate[0][ii]>highFreq){
		                high=ii-1;
		                if(high<0){
		                    logger.debug("plotPowerSpectrum - highFreq out of range -  reset to highest value");
    		                high = n;
	    	            }
		                test = false;
		            }
    ...
    public void plotPowerLog(double lowFreq, double highFreq, String graphTitle){
            ...
            int high = 0;
            if(!this.deltaTset){
                logger.debug("plotPowerLog - deltaT has not been set");
                logger.debug("full spectrum plotted");
		    }
		    else{
		        int ii = 0;
    		    ...
    		    if(lowFreq==-1.0D){
		            ...
		            while(test){
    		            if(this.powerSpectrumEstimate[0][ii]>lowFreq){
    		                ...
    		                if(ii>=n){
	    	                    low = 0;
		                        logger.debug("plotPowerLog - lowFreq out of range -  reset to zero");
    		                    test = false;
	    	                }
		                }
    		        }
		        ...
		        while(test){
		            if(this.powerSpectrumEstimate[0][ii]>highFreq){
		                high=ii-1;
		                if(high<0){
		                    logger.debug("plotPowerLog - highFreq out of range -  reset to highest value");
    		                high = n;
	    	            }
		                test = false;
		            }
    ...
    private void plotPowerSpectrumLog(int low, int high, String graphTitle){
            ...
            while(test){
                if(spectrum[1][ii]>0.0D){
                    ...
                    if(ii>=nNew){
                        test = false;
                        logger.debug("plotPowerSpectrumLog:  no non-zero amplitudes");
                        System.exit(0);
                    }
                }
            }
...
*/
    ...
    public double[][]getCorrelation(){
        if(!this.correlateDone){
            logger.debug("getCorrelation - correlation has not been called - no correlation returned");
		}
        return this.correlationArray;
	}
    ...
    public void printCorrelation(String filename){
        if(!this.correlateDone){
            logger.debug("printCorrelation - correlate has not been called - no file printed");
		}
		else{
		    FileOutput fout = new FileOutput(filename);
    ...
    public void plotCorrelation(){
        if(!this.correlateDone){
            logger.debug("plotCorrelation - correlation has not been called - no plot displayed");
		}
		else{
    ...
    public void plotCorrelation(String graphTitle){
        if(!this.correlateDone){
            logger.debug("plotCorrelation - correlate has not been called - no plot displayed");
		}
		else{
	...
	*/
	...
	public double[][] shortTime(double windowTime){
	    ...
	    if(!FourierTransform.checkPowerOfTwo(windowLength)){
            ...
            logger.debug("Method - shortTime");
            logger.debug("Window length, provided as time, "+windowTime+", did not convert to an integer power of two data points");
            logger.debug("A value of "+((windowLength-1)*this.deltaT)+" was substituted");
        }
    ...
    public void printShortTime(String filename){
        if(!this.shortTimeDone){
            logger.debug("printShortTime- shortTime has not been called - no file printed");
		}
		else{
		    FileOutput fout = new FileOutput(filename);
